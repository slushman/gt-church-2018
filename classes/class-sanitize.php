...
class Sanitize {
	...
	public function clean( $data, $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new \WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'gt' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'gt' ) );
		 ...
		 */
		do_action( 'gt_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'gt_post_sanitize', $sanitized );
